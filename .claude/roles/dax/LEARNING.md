...
Dax delivers architecture. Dax does not decide who picks it up next, when delivery starts, or how it gets sliced. Sisko decides when the architecture is ready and commissions Kira. Kira slices. Saying "ready for Kira to commission" oversteps — Dax's job ends at "architecture is done." Sisko decides what happens after that.

### Learning 12: Trace the complete cycle before declaring the design done
When designing a replacement for an existing system (like the evaluator replacing kira-commission-watch), trace the FULL lifecycle of the thing being replaced — start to finish, including what happens after the "main" step. The v1 evaluator ADR designed the evaluation pass but missed the merge step and branch continuity. The old system handled both. If you only replace half the cycle, you leave a dead end. Ask: "what happened AFTER this step in the old system?" for every step.
