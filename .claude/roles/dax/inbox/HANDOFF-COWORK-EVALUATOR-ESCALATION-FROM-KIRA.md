...
- `bridge/orchestrator.js` — the relay. Polls queue, invokes `claude -p`, writes heartbeat. It already runs `claude -p` for commissions — it could also run `claude -p` for evaluation.
...
1. Does the evaluator run inside `orchestrator.js` (extending it) or as a separate process?
