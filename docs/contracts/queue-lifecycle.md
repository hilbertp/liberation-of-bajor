...
|---|---|---|
| `PENDING` | `{id}-PENDING.md` | Commission written by Kira, waiting to be picked up. |
...
Kira assigns IDs when writing new commissions. The rule:
...
| `{id}-IN_PROGRESS.md` with `{id}-ERROR.md` present | Delete IN_PROGRESS. Commission failed — Kira must evaluate. |
...
After writing a commission with a known ID, Kira polls for:
